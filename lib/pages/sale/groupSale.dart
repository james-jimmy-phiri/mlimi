...
class GroupSalePage extends StatefulWidget {
  ...
  const GroupSalePage({this.onSubmitted, super.key});

  @override
  State<GroupSalePage> createState() => _GroupSalePageState();
}

class _GroupSalePageState extends State<GroupSalePage> {
  late String unitPrice, quantity, description, expectedSupplyDate;
  // Initializing the contributions list
  ...
  List<Map<String, dynamic>> contributionInputs = [
    {'member_id': null, 'controller': TextEditingController()},
  ];

  List<dynamic> members = GetStorage().read('members') ?? [];
  ...
  Future<void> submit() async {
    if (validate()) {
      ...
      contributions = contributionInputs
          .where((entry) =>
              entry['member_id'] != null && entry['controller'].text.isNotEmpty)
          .map((entry) => {
                'member_id': entry['member_id'],
                'quantity': double.tryParse(entry['controller'].text) ?? 0,
              })
          .toList();
      ...
      try {
        var request = http.MultipartRequest('POST', url)
          ...
          ..fields['contributions'] = jsonEncode(contributions);
        ...
        print('🛰️ Submitting request with the following fields:');
